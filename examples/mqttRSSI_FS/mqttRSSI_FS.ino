...
// Loads the AutoConnectAux JSON description in bulk from the filesystem.
bool loadAux(void) {
  // The JSON file name of the custom web page equals the URL assigned to each
  // page with `.json` extension appended.
  static const char*  customPages[] = {
    URL_MQTT_SETTING,
    URL_MQTT_START,
    URL_MQTT_CLEAR,
    URL_MQTT_STOP
  ...
  for (unsigned int page = 0U; page < sizeof(customPages) / sizeof(char*); page++) {
    String  fn = String(customPages[page]) + ".json";
    Serial.printf("Loading %s ...", fn.c_str());
    File  jsonFile = FlashFS.open(fn.c_str(), "r");
    if (jsonFile) {
      if (portal.load(jsonFile))
...
void setup() {
  ...
  // This example illustrates the procedure for reading a JSON description from
  // the filesystem that defines a custom web page.
  // `loadAux` function loads the AutoConnectAux JSON description in bulk from
  // the filesystem.
  if (!loadAux())
    return;
