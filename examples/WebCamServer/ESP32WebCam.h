...
// URL that respond for capturing
#ifndef ESP32CAM_DEFAULT_URL_CAPTURE
#define ESP32CAM_DEFAULT_URL_CAPTURE  "/_capture"
#endif // !ESP32CAM_DEFAULT_URL_CAPTURE

// URL that prompts ESP32Cam to invoke the function
#ifndef ESP32CAM_DEFAULT_URL_PROMPT
#define ESP32CAM_DEFAULT_URL_PROMPT   "/_prompt"
#endif // !ESP32CAM_DEFAULT_URL_PROMPT

// URL that respond for streaming
#ifndef ESP32CAM_DEFAULT_URL_STREAM
#define ESP32CAM_DEFAULT_URL_STREAM   "/_stream"
#endif // !ESP32CAM_DEFAULT_URL_STREAM
...
#define ESP32CAM_PROMPT_OPERAND_VALUE_SDMMC         "sdmmc"
...
class ESP32WebCam {
 public:
  ...
  const char* getPromptUrl(void) const { return _promptUrl.c_str(); }
  const char* getStreamUrl(void) const { return _streamUrl.c_str(); }
  ...
  void      setCaptureUrl(const char* url) { _captureUrl = url; }
  void      setPromptUrl(const char* url) { _promptUrl = url; }
  void      setStreamUrl(const char* url) { _streamUrl = url; }
  ...
  esp_err_t startCameraServer(const char* streamUrl) { return startCameraServer(streamUrl, nullptr, nullptr, _port); }
  esp_err_t startCameraServer(const char* streamUrl, const uint16_t port) { return startCameraServer(streamUrl, nullptr, nullptr, port); }
  esp_err_t startCameraServer(const char* streamUrl, const char* captureUrl, const char* promptUrl) { return startCameraServer(streamUrl, captureUrl, promptUrl, _port); }
  esp_err_t startCameraServer(const char* streamUrl, const char* captureUrl, const char* promptUrl, const uint16_t port);
 ...
 private:
  ...
  String _captureUrl;         /**< URL to reply with a one-shot JPEG image */
  String _promptUrl;          /**< URL to prompt the ESP32Cam instance to perform the function */
  String _streamUrl;          /**< URL to play continuous Motion-JPEG frames. */
