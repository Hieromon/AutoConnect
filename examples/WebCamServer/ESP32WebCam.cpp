 ...
 * @param  streamUrl  Specifies the URL of an endpoint that allows motion JPEG streaming.
 * @param  captureUrl Specifies the URL of an endpoint that allows taking a still image.
 * @param  promptUrl  Specifies the URL of an endpoint that allows remote controlling.
 * @param  port       HTTP port number
 * @return ESP_OK
 * @return ESP_ERR_HTTPD_TASK
 */
esp_err_t ESP32WebCam::startCameraServer(const char* streamUrl, const char* captureUrl, const char* promptUrl, const uint16_t port) {
  esp_err_t rc;

  // If streamUrl is null, the default endpoint will be applied.
  if (!streamUrl) {
    if (!_streamUrl.length())
      setStreamUrl(ESP32CAM_DEFAULT_URL_STREAM);
  }

  // If captureUrl is null, the default endpoint will be applied.
  if (!captureUrl) {
    if (!_captureUrl.length())
      setCaptureUrl(ESP32CAM_DEFAULT_URL_CAPTURE);
  }

  // If promptUrl is null, the default endpoint will be applied.
  if (!promptUrl) {
    if (!_promptUrl.length())
      setPromptUrl(ESP32CAM_DEFAULT_URL_PROMPT);
  }
  ...
  if (rc == ESP_OK) {
    ...
    if (rc != ESP_OK)
      ESP_LOGE(ESP32CAM_LOGE_TAG, "%s handler could not register 0x%04x", _captureUrl.c_str(), rc);
    rc = httpd_register_uri_handler(_stream_d, &promptUri);
    if (rc != ESP_OK)
      ESP_LOGE(ESP32CAM_LOGE_TAG, "%s handler could not register 0x%04x", _promptUrl.c_str(), rc);
    rc = httpd_register_uri_handler(_stream_d, &streamUri);
    if (rc != ESP_OK)
      ESP_LOGE(ESP32CAM_LOGE_TAG, "%s handler could not register 0x%04x", _streamUrl.c_str(), rc);
  }
  else
...
esp_err_t ESP32WebCam::_captureHandler(httpd_req_t* req) {
  ...
  if (fb) {
    if (fb->format == PIXFORMAT_JPEG) {
      ...
      // Respond with plain error message content if the capture fails.
      _sendResponse(req, HTTPD_500_INTERNAL_SERVER_ERROR, "text/plain", "ESP_ERR_CAMERA_FAILED_TO_SET_OUT_FORMAT");
      rc = ESP_ERR_CAMERA_FAILED_TO_SET_OUT_FORMAT;
    }
 ...
 * mf=oneshot[&fs={SD|SDMMC}][&filename=FILENAME]
 * - Periodically shoot and export to SD or SD_MMC with the captured jpeg image files.
 * mf=timershot[&fs={SD|SDMMC}][&filename=FILENAME][&period=SHOOTING TIME INTERVAL]
  ...
  if (qLen > 0) {
    ...
    if ((rc = httpd_req_get_url_query_str(req, query, sizeof(query))) == ESP_OK) {
      ...
      if (!strcmp(ESP32CAM_PROMPT_OPERAND_VALUE_ONESHOT, mf)) {
        if (!strcmp(ESP32CAM_PROMPT_OPERAND_VALUE_SDMMC, fs) || !strlen(fs))
...
esp_err_t ESP32WebCam::_streamHandler(httpd_req_t* req) {
  ...
  while (c < sizeof(boundary) - 1)
    boundary[c++] = bcharsnospace[(int)_random(sizeof(bcharsnospace))];
  boundary[c] = '\0';
  ...
  while (rc == ESP_OK) {
    ...
    if ((rc = httpd_resp_send_chunk(req, contentBoundary, sizeof(contentBoundary))) == ESP_OK) {
      ...
      ESP_LOGV(ESP32CAM_LOGE_TAG, "session closed by peer");
    }
    if (fb) {
      esp_camera_fb_return(fb);
      fb = nullptr;
