...
// _cameraServerPort is the port of the webserver that the ESP32WebCam class
// start on will listen on.
const uint16_t _cameraServerPort = 3000;
...
// - platformio.ini
//   build_flags =
//     -DESP32CAM_DEFAULT_URL_CAPTURE='"path you wish to CAPTURE's endpoint"'
//     -DESP32CAM_DEFAULT_URL_PROMPT='"path you wish to PROMPT's endpoint"'
//     -DESP32CAM_DEFAULT_URL_STREAM='"path you wish to STREAM's endpoint"'
const char*  const _captureUrl = ESP32CAM_DEFAULT_URL_CAPTURE;
const char*  const _streamUrl = ESP32CAM_DEFAULT_URL_STREAM;
const char*  const _promptUrl = ESP32CAM_DEFAULT_URL_PROMPT;
...
// - stream=STREAMING_URL                     (default: _stream)
// - capture=CAPTURING_URL                    (default: _capture)
// - prompt=PROMPTING_URL_FOR_REMOTE_CONTROL  (default: _prompt)
// - setting=AUTOCONNECTAUX_SETTING_URL       (default: undefined)
// - ac=AUTOCONNECT_ROOT_URL                  (default: _ac) 
...
String viewerUrl(void) {
  String  url = String(_viewerUrl)
              ...
              + "&port=" + String(_cameraServerPort)
              + "&setting=" + _sensorUrl
              + "&capture=" + String(_captureUrl)
              + "&prompt=" + String(_promptUrl)
              + "&stream=" + String(_streamUrl)
              + "&ac=" AUTOCONNECT_URI
...
void setup() {
  ...
  if (err != ESP_OK)
    ...
    // As long as you adopt the default as the endpoint required by the
    // ESP32WebCam class, the following steps are unnecessary.
    webCam.setCaptureUrl(_captureUrl);
    webCam.setPromptUrl(_promptUrl);
    webCam.setStreamUrl(_streamUrl);
  }
  ...
  if (portal.begin()) {
    ...
    if (err == ESP_OK) {
      ...
      Serial.printf("Camera streaming %s:%u server ready\n", WiFi.localIP().toString().c_str(), _cameraServerPort);
    }
    else
