...
/**
 * Take a one-shot image and save it to the SDMMC card. 
 * This function writes to a file compliant with the following SD library,
 * and The SD card needs to be connected to the ESP32 module via HS2 wiring.
 * https://github.com/espressif/arduino-esp32/tree/master/libraries/SD
 * @param  sdmmc     fs::SDMMCFS instance that handles the destination SD card.
 * @param  filename  File name for saving the captured image.
 * @return ESP_OK    Image file saved.
 * @return ESP_ERR_INVALID_STATE  SD card is not mounted.
 * @return ESP_FAIL  Any other error occurred.
 */
esp_err_t ESP32Cam::oneShot(fs::SDMMCFS& sdmmc, const char* filename) {
  ...
  if (!_autoMount(MOUNT_SDMMC, reinterpret_cast<fs::FS*>(&sdmmc)))
    return ESP_ERR_INVALID_STATE;
  return _oneShot(filename);
}
...
esp_err_t ESP32Cam::_oneShot(const char* filename) {
  ...
  if (xSemaphoreTake(ESP32Cam_internal::xMutex, portMAX_DELAY) == pdTRUE) {
...
/**
 * Repeatedly takes a one-shot image at a specified cycle and saves it to
 * the SD card. Saves the captured image to a file whose name consists of
 * the prefix and timestamp specified by filenamePrefix parameter. If the
 * filename parameter is nullptr, it will assume a file name with
 * a timestamp prefixed with a fixed string defined by
 * the ESP32CAM_GLOBAL_IDENTIFIER macro.
 * This function writes to a file compliant with the following SD library,
 * and the SD card must be connected to the ESP32 module via SPI.
 ...
 * https://github.com/espressif/arduino-esp32/tree/master/libraries/SD
 * @param  period         Period to shoot in seconds.
 * @param  sdmmc          fs::SDMMC filesystem
 * @param  filenamePrefix File name for saving the captured image.
 * @return ESP_OK         Image file saved.
 * @return ESP_FAIL       Any other error occurred.
 */
esp_err_t ESP32Cam::timerShot(const unsigned long period, fs::SDMMCFS& sdmmc, const char* filenamePrefix) {
  // Allow one-shot writes even if SDMMC.begin() is not running in the user
  // sketch. To do so, it detects the card mount state only once and tries to
  // start the SDMMC file system.
  if (!_autoMount(MOUNT_SDMMC, reinterpret_cast<fs::FS*>(&sdmmc)))
...
/**
 * A hardware timer interrupt routine that is called when the shooting cycle of
 * the timerShot is reached. Since this ISR needs to finish with minimal
 * processing, it only locks a Mutex resource and spawns a task to perform the
 * one-shot.
 */
...
/**
 * The timer shot operation runs as a task generated by the interrupt routine.
...
bool ESP32Cam::_autoMount(const SDType_t sdType, fs::FS* sdFile) {
  ...
  case MOUNT_SD: {
    ...
    if (sd->cardType() == CARD_NONE)
      ...
      if (!sd->open("/" ESP32CAM_GLOBAL_IDENTIFIER, FILE_WRITE)) {
        ESP_LOGV("SD mount point may have been removed, remounting\n");
        sd->end();
        rc = sd->begin();
      }
      else
        rc = sd->remove("/" ESP32CAM_GLOBAL_IDENTIFIER);
    }
    if (rc)
      _sdFile = sd;
    break;
  ...
  case MOUNT_SDMMC: {
    ...
    if (sdmmc->cardType() == CARD_NONE)
      ...
      if (!sdmmc->open("/" ESP32CAM_GLOBAL_IDENTIFIER, FILE_WRITE)) {
        ESP_LOGV("SDMMC mount point may have been removed, remounting\n");
        sdmmc->end();
        rc = sdmmc->begin();
      }
      else
        rc = sdmmc->remove("/" ESP32CAM_GLOBAL_IDENTIFIER);
    }
    if (rc)
      _sdFile = sdmmc;
    break;
